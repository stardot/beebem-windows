...
// Set chip model.
// ----------------------------------------------------------------------------
void SIDChip::set_chip_model(chip_model model)
{
  for (int i = 0; i < 3; i++) {
...
// Write registers.
// ----------------------------------------------------------------------------
void SIDChip::write(reg8 offset, reg8 value)
{
  bus_value = value;
...
// Read state.
// ----------------------------------------------------------------------------
SIDChip::State SIDChip::read_state()
{
  State state;
  int i, j;
...
// Write state.
// ----------------------------------------------------------------------------
void SIDChip::write_state(const State& state)
{
  int i;
...
// Enable filter.
// ----------------------------------------------------------------------------
void SIDChip::enable_filter(bool enable)
{
...
// ----------------------------------------------------------------------------
void SIDChip::enable_external_filter(bool enable)
{
...
// not overfilled.
// ----------------------------------------------------------------------------
bool SIDChip::set_sampling_parameters(double clock_freq, sampling_method method,
...
// ----------------------------------------------------------------------------
void SIDChip::adjust_sampling_frequency(double sample_freq)
{
  cycles_per_sample =
...
// ----------------------------------------------------------------------------
void SIDChip::fc_default(const fc_point*& points, int& count)
{
...
// ----------------------------------------------------------------------------
PointPlotter<sound_sample> SIDChip::fc_plotter()
{
...
// ----------------------------------------------------------------------------
void SIDChip::clock(cycle_count delta_t)
{
  int i;
...
// 
// ----------------------------------------------------------------------------
int SIDChip::clock(cycle_count& delta_t, short* buf, int n, int interleave)
{
  switch (sampling) {
  default:
  case SAMPLE_FAST:
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDChip::clock_fast(cycle_count& delta_t, short* buf, int n,
		    int interleave)
{
  int s = 0;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDChip::clock_interpolate(cycle_count& delta_t, short* buf, int n,
			   int interleave)
{
  int s = 0;
  int i;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDChip::clock_resample_interpolate(cycle_count& delta_t, short* buf, int n,
				    int interleave)
{
  int s = 0;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDChip::clock_resample_fast(cycle_count& delta_t, short* buf, int n,
			     int interleave)
{
  int s = 0;
