...
void EconetReset()
{
	...
	if (MachineType == Model::FileStoreE01 || MachineType == Model::FileStoreE01S) {
		CMOSWrite(0xe, EconetStationID);
		CMOSWrite(0xf, 0xff-EconetStationID); // checkbyte
	}
...
bool EconetPoll_real() // return NMI status
{
					    ...
					    fourwaystage == FourWayStage::DataSent)
					{
						...
						if (RetVal > 0)
						{
							...
							if (RetVal > 0)
							{
								if (DebugEnabled)
								{
									ECOLAN* temp1 = FindHost(&RecvAddr);

									DebugDisplayTraceF(DebugType::Econet, true,
									                   "EconetPoll: Packet received: %u bytes from network %d station %d (%s port %u)",
									                   (int)RetVal,
										(unsigned int)temp1->network,
										(unsigned int)temp1->station,
									                   IpAddressStr(RecvAddr.sin_addr.s_addr),
	...
	if (!(ADLC.control1 & CONTROL_REG1_TX_RESET)) // not txreset
	{
		if (!(ADLC.control2 & CONTROL_REG2_TDRA_SELECT)) // tdra mode
		{
			    ...
			    && (!(ADLC.status2 & STATUS_REG2_DCD)) ) // DTR not high
			{
				if (!(ADLC.status1 & STATUS_REG1_TDRA))
					if (DebugEnabled) DebugDisplayTrace(DebugType::Econet, true, "set tdra");
			...
			{
				if ((ADLC.status1 & STATUS_REG1_TDRA))
					if (DebugEnabled) DebugDisplayTrace(DebugType::Econet, true, "clear tdra");
		...
		else // FC mode
		{
			...
			{
				if (!(ADLC.status1 & STATUS_REG1_TDRA))
					if (DebugEnabled) DebugDisplayTrace(DebugType::Econet, true, "set fc");
			...
			{
				if ((ADLC.status1 & STATUS_REG1_TDRA))
					if (DebugEnabled) DebugDisplayTrace(DebugType::Econet, true, "clear fc");
